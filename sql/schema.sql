...
CREATE TABLE IF NOT EXISTS public.user_challenge_progress (
    ...
    UNIQUE(user_id, challenge_id, last_updated_date)
);
...
CREATE OR REPLACE FUNCTION handle_challenge_action(
    p_user_id uuid,
    p_action_type TEXT
)
RETURNS void
...
DECLARE
    ...
    v_hour INT := EXTRACT(HOUR FROM CURRENT_TIMESTAMP);
    v_challenge_id TEXT;
    v_challenge_points INT;
    v_challenge_target INT;
    v_new_progress INT;
    v_is_completed BOOLEAN;
BEGIN
    IF p_action_type = 'post' THEN
        v_challenge_id := 'post_positive';
        v_challenge_points := 10;
        v_challenge_target := 1;
    ELSIF p_action_type = 'comment' THEN
        v_challenge_id := 'help_others';
        v_challenge_points := 15;
        v_challenge_target := 3;
    ELSIF p_action_type = 'reaction' THEN
        v_challenge_id := 'react_to_posts';
        v_challenge_points := 5;
        v_challenge_target := 5;
    ELSIF p_action_type = 'poll' THEN
        v_challenge_id := 'use_poll';
        v_challenge_points := 10;
        v_challenge_target := 1;
    ELSE
        RETURN;
    ...
    INSERT INTO public.user_challenge_progress (user_id, challenge_id, progress_count, completed, last_updated_date)
    VALUES (p_user_id, v_challenge_id, 1, false, v_today)
    ON CONFLICT (user_id, challenge_id, last_updated_date)
    DO UPDATE
        SET progress_count = user_challenge_progress.progress_count + 1
        WHERE user_challenge_progress.completed = false
    RETURNING progress_count, completed INTO v_new_progress, v_is_completed;

    IF NOT v_is_completed AND v_new_progress >= v_challenge_target THEN
            UPDATE public.user_challenge_progress
            SET completed = true
            WHERE user_id = p_user_id AND challenge_id = v_challenge_id AND last_updated_date = v_today;

        INSERT INTO public.user_points (user_id, total_points)
        VALUES (p_user_id, v_challenge_points)
        ON CONFLICT (user_id)
        DO UPDATE
            SET total_points = user_points.total_points + v_challenge_points;
    END IF;

    IF p_action_type = 'post' THEN
        IF (v_hour >= 22 OR v_hour <= 2) THEN
            v_challenge_id := 'evening_post';
            v_challenge_points := 8;

            INSERT INTO public.user_challenge_progress (user_id, challenge_id, progress_count, completed, last_updated_date)
            VALUES (p_user_id, v_challenge_id, 1, true, v_today)
            ON CONFLICT (user_id, challenge_id, last_updated_date) DO NOTHING;

            IF FOUND THEN
                INSERT INTO public.user_points (user_id, total_points)
                VALUES (p_user_id, v_challenge_points)
                ON CONFLICT (user_id)
                DO UPDATE SET total_points = user_points.total_points + v_challenge_points;
            END IF;
        END IF;

        IF (v_hour >= 6 AND v_hour <= 9) THEN
            v_challenge_id := 'morning_post';
            v_challenge_points := 8;
