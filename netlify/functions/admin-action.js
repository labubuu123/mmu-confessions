// admin-action: verifies admin via Supabase auth token and uses service role to update/delete
...
async function verifyAdmin(accessToken) {
    ...
    const uid = user?.id
    if (!uid) return null
    // check admins table using service key
    ...
    if (action === 'approve' || action === 'unapprove') {
        ...
        if (!res.ok) return { statusCode: 500, body: `Failed to update: ${await res.text()}` }
