...
import { ChevronLeft, User, Heart, Loader2, Ban, CheckCircle, Search, X, ShieldAlert } from 'lucide-react'
    ...
    return (
        <div className="space-y-4 md:space-y-6 pb-20">
            <div className="flex flex-col gap-3 md:gap-4">
                ...
                <div className="flex w-full md:w-auto p-1 bg-gray-100 dark:bg-gray-800 rounded-xl self-start">
                    <button
                        onClick={() => setViewMode('all')}
                        className={`flex-1 md:flex-none justify-center px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition flex items-center gap-2 ${viewMode === 'all'
                                ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm'
                                : 'text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200'
                            }`}
                    >
                        All Users
                        <span className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-600 rounded text-[10px] sm:text-xs">
                            {users.length}
                        </span>
                    </button>
                    <button
                        onClick={() => setViewMode('blocked')}
                        className={`flex-1 md:flex-none justify-center px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition flex items-center gap-2 ${viewMode === 'blocked'
                    ...
                    >
                        ...
                        Blocked
                        <span className={`px-1.5 py-0.5 rounded text-[10px] sm:text-xs ${viewMode === 'blocked' ? 'bg-white/20 text-white' : 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400'
                            }`}>
                            {blockedCount}
                        </span>
                    </button>
                            ...
                            <div className="space-y-3 flex-1">
                                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                                    <div className="bg-indigo-50 dark:bg-indigo-900/20 p-2 rounded-lg text-center">
                                        <div className="text-lg sm:text-xl font-bold text-indigo-600 dark:text-indigo-400">{user.post_count}</div>
                                        <div className="text-[10px] sm:text-xs text-indigo-800 dark:text-indigo-300 font-medium">Posts</div>
                                    </div>
                                    <div className="bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg text-center">
                                        <div className="text-lg sm:text-xl font-bold text-blue-600 dark:text-blue-400">{user.comment_count}</div>
                                        <div className="text-[10px] sm:text-xs text-blue-800 dark:text-blue-300 font-medium">Comments</div>
                                    </div>
                                </div>

                                <div className="flex items-center justify-between text-xs sm:text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 p-2 rounded-lg">
