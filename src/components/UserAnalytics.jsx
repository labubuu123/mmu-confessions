...
import {
    ...
    CalendarDays, BarChart2, Star, Activity, ArrowRight
} from 'lucide-react';
...
export default function UserAnalytics() {
    ...
    const { karmaPoints, currentStreak, isLoading: karmaLoading } = useKarma(anonId);
    ...
    async function fetchAnalyticsData() {
        ...
        try {
            const [postsRes, commentsRes] = await Promise.all([
                supabase.from('confessions').select('id, created_at, likes_count, comments_count, text, tags').eq('author_id', anonId).order('created_at', { ascending: false }),
                supabase.from('comments').select('id, created_at').eq('author_id', anonId)
            ]);
            ...
            let events = [];
            if (posts.length > 0) {
                const { data } = await supabase.from('events').select('id, confession_id').in('confession_id', posts.map(p => p.id));
                events = data || [];
            }
            ...
            const heatmapData = Array.from({ length: 60 }, (_, i) => {
                const d = dayjs().subtract(59 - i, 'day');
                return { date: d.format('YYYY-MM-DD'), count: dateMap[d.format('YYYY-MM-DD')] || 0 };
            });
            ...
            setPersona(determinePersona(posts, comments.length, hours, totalLikes));
            setBadges(calculateBadges(posts, events));
    ...
    function determinePersona(posts, commentCount, hours, totalLikes) {
        if (!posts.length && !commentCount) return { title: "The Ghost", icon: Ghost, color: "text-gray-400", bg: "bg-gray-100 dark:bg-gray-800", desc: "Lurking in the shadows." };

        const nightActivity = hours.slice(0, 5).reduce((a, b) => a + b, 0) + hours.slice(22, 24).reduce((a, b) => a + b, 0);
        const totalActivity = hours.reduce((a, b) => a + b, 0) || 1;

        if (posts.length && (totalLikes / posts.length) > 15) return { title: "Campus Celebrity", icon: Crown, color: "text-yellow-500", bg: "bg-yellow-50 dark:bg-yellow-900/20", desc: "Your posts run this campus." };
        if (posts.filter(p => p.tags?.includes('debate')).length > 2) return { title: "The Devil's Advocate", icon: Flame, color: "text-orange-500", bg: "bg-orange-50 dark:bg-orange-900/20", desc: "Peace was never an option." };
        if ((nightActivity / totalActivity) > 0.4) return { title: "The Night Owl", icon: Moon, color: "text-indigo-500", bg: "bg-indigo-50 dark:bg-indigo-900/20", desc: "Alive when the campus sleeps." };
        if (posts.filter(p => p.tags?.some(t => ['rant', 'angry'].includes(t))).length > 3) return { title: "The Vent Master", icon: Zap, color: "text-red-500", bg: "bg-red-50 dark:bg-red-900/20", desc: "You have a lot of feelings." };
        if (commentCount > posts.length * 3) return { title: "The Yapologer", icon: MessageSquare, color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-900/20", desc: "Everywhere in the comments." };
    ...
    if (!stats || (!stats.totalPosts && !stats.totalComments)) {
        return (
            <div className="text-center py-20 px-4">
                ...
                <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6">Start posting confessions to unlock analytics!</p>
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-4 md:py-8 pb-24 space-y-4 md:space-y-6">
            <Helmet><title>My Stats - MMU Confessions</title></Helmet>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div className="flex-1">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Your anonymous footprint</p>
                </div>
                ...
                <div className="w-full md:w-auto bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-1 rounded-xl shadow-lg hover:shadow-indigo-500/20 transition-all">
                    <div className="flex items-center justify-between gap-4 px-3 py-2 bg-white/10 backdrop-blur-sm rounded-lg">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] font-bold uppercase opacity-80 tracking-wider">Total Points</p>
                                <p className="text-xl md:text-2xl font-black leading-none">{karmaPoints.toLocaleString()}</p>
                            </div>
                        </div>
                        {/* Here is the new button to navigate to the Check-in page */}
                        <Link to="/karma-shop" className="text-xs font-bold bg-white text-indigo-600 px-3 py-1.5 rounded-lg flex items-center gap-1 active:scale-95 transition-transform">
                            Karma Hub <ArrowRight className="w-3 h-3" />
                        </Link>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                    ...
                    <div className="col-span-2 md:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden relative p-5 md:p-6 flex flex-col justify-center min-h-[140px]">
                        ...
                        <div className="flex items-center gap-4 md:gap-5 relative z-10">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600 dark:text-gray-300 text-xs md:text-sm font-medium leading-relaxed">"{persona.desc}"</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <StatBox icon={Zap} label="Day Streak" value={currentStreak} color="text-orange-500" bg="bg-orange-50 dark:bg-orange-900/20" />
                    ...
                    <div className="overflow-x-auto no-scrollbar pb-1">
                        <div className="flex md:flex-wrap gap-1 min-w-max md:min-w-0">
                            {stats.heatmapData.map((day, i) => (
                                <div key={i} className={`w-3 h-3 md:w-4 md:h-4 rounded-[3px] md:rounded-sm flex-shrink-0 transition-all ${day.count === 0 ? 'bg-gray-100 dark:bg-gray-700/50' :
                                    day.count < 2 ? 'bg-indigo-300 dark:bg-indigo-800' :
                                        day.count < 4 ? 'bg-indigo-500 dark:bg-indigo-600' : 'bg-indigo-700 dark:bg-indigo-400'
                                    }`} />
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex items-end justify-between h-20 gap-0.5">
                        {stats.hourlyData.filter((_, i) => i % 2 === 0).map((count, i) => (
                            <div key={i} className="flex-1 bg-indigo-50 dark:bg-gray-700/30 rounded-sm relative group">
                                <div style={{ height: `${Math.max((count / Math.max(...stats.hourlyData, 1)) * 100, 15)}%` }}
                                    className={`absolute bottom-0 w-full rounded-sm ${i > 3 && i < 9 ? 'bg-indigo-300 dark:bg-indigo-600' : 'bg-indigo-500 dark:bg-indigo-400'}`} />
                            </div>
                        ))}
                    </div>
                    <div className="flex justify-between text-[10px] text-gray-400 mt-2 font-mono uppercase"><span>12am</span><span>12pm</span></div>
                </div>
                ...
                {stats.mostPopularPost && (
                    <div className="col-span-2 md:col-span-3 lg:col-span-2 bg-gradient-to-br from-gray-900 to-gray-800 dark:from-gray-800 dark:to-gray-900 rounded-2xl p-5 text-white shadow-md flex flex-col justify-between min-h-[140px]">
                        <div>
                            ...
                            <p className="text-sm md:text-base font-medium italic opacity-90 line-clamp-2 md:line-clamp-3">"{stats.mostPopularPost.text}"</p>
                        </div>
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-4 md:p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col justify-between h-full">
            <div className="flex items-start justify-between mb-2">
                <div className={`p-2 rounded-lg ${bg}`}><Icon className={`w-4 h-4 md:w-5 md:h-5 ${color}`} /></div>
            </div>
            <div>
