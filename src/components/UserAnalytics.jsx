        ...
        <>
            ...
            <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-2 rounded-xl shadow-lg flex items-center gap-2">
                        <Award className="w-5 h-5 text-yellow-300" />
                        <div>
                            <p className="text-[10px] font-bold uppercase opacity-80 tracking-wider">Karma Score</p>
                            <p className="text-xl font-black leading-none">{stats.karmaScore}</p>
                        </div>
                    </div>
                </div>
                ...
                {persona && (
                    <div className="bg-white dark:bg-gray-800 rounded-2xl border-2 border-indigo-100 dark:border-indigo-900 shadow-xl overflow-hidden relative">
                        ...
                        <div className="p-6 sm:p-8 relative z-10 flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
                            <div className={`w-24 h-24 sm:w-32 sm:h-32 rounded-full ${persona.bg} flex items-center justify-center shadow-inner`}>
                                <persona.icon className={`w-12 h-12 sm:w-16 sm:h-16 ${persona.color}`} />
                            </div>
                            <div className="flex-1">
                                <div className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-1">Your Persona</div>
                                <h2 className={`text-3xl sm:text-4xl font-black ${persona.color} mb-2`}>{persona.title}</h2>
                                <p className="text-gray-600 dark:text-gray-300 text-lg font-medium leading-relaxed max-w-xl">
                                    "{persona.desc}"
                                </p>
                            </div>
                        </div>
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
                        ...
                        <div className="flex flex-wrap gap-1 content-start justify-center sm:justify-start h-32 overflow-hidden">
                            {stats.heatmapData.map((day, i) => (
                                <div
                                    key={i}
                                    title={`${day.date}: ${day.count} activities`}
                                    className={`w-3 h-3 sm:w-4 sm:h-4 rounded-sm transition-all duration-300 hover:scale-125 hover:ring-2 ring-indigo-300 ${day.count === 0 ? 'bg-gray-100 dark:bg-gray-700' :
                            ...
                            {stats.hourlyData.map((count, hour) => {
                                ...
                                return (
                                    <div key={hour} className="flex-1 flex flex-col justify-end group relative">
                                        ...
                                        />
                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 bg-black text-white text-[10px] px-1 py-0.5 rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-10">
                                            {hour}:00 - {count}
                                        </div>
                                    </div>
                                )
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                            {badges.map((badge, idx) => (
                                <div key={idx} className="flex flex-col items-center p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl border border-gray-100 dark:border-gray-700 text-center transition hover:scale-105">
                                    ...
                                    <span className="text-[10px] text-gray-500 dark:text-gray-400 leading-tight">{badge.description}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {stats.mostPopularPost && (
                    <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-6 text-white shadow-xl">
                        ...
                        <p className="text-lg sm:text-xl font-medium italic mb-6 line-clamp-3">
                            "{stats.mostPopularPost.text}"
                        </p>
                        <div className="flex gap-4 text-sm font-bold opacity-90">
                            <span className="flex items-center gap-1"><Heart className="w-4 h-4" /> {stats.mostPopularPost.likes_count} Likes</span>
                            <span className="flex items-center gap-1"><MessageSquare className="w-4 h-4" /> {stats.mostPopularPost.comments_count} Comments</span>
                        </div>
                    </div>
                )}
            </div>
...
function StatBox({ icon: Icon, label, value, color }) {
    return (
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm flex items-center gap-4">
            <div className={`p-3 rounded-xl bg-gray-50 dark:bg-gray-700 ${color}`}>
