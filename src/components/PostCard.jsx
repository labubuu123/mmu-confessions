    ...
    return (
        <>
            <div
                ...
                className="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-md hover:shadow-2xl border border-gray-200 dark:border-gray-700 transition-all duration-300 cursor-pointer mb-3 sm:mb-4 overflow-hidden group relative"
            >
                <div className="absolute top-1.5 sm:top-3 right-1.5 sm:right-3 z-10 flex flex-col items-end gap-1 sm:gap-2 max-w-[calc(100%-3rem)] sm:max-w-[calc(100%-4rem)]">
                    {postAge && (
                        <span className="px-1.5 py-0.5 sm:px-2 sm:py-0.5 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 text-[10px] sm:text-xs rounded-full font-medium flex items-center gap-0.5 sm:gap-1 whitespace-nowrap">
                    ...
                    {post.pinned && (
                        <div className="px-1.5 py-0.5 sm:px-3 sm:py-1 bg-blue-600 text-white text-[10px] sm:text-xs font-bold rounded-full shadow-lg flex items-center gap-0.5 sm:gap-1 whitespace-nowrap">
                            📌 <span className="hidden xs:inline">PINNED</span>
                        </div>
                    )}

                    {isHotPost && (
                        <div className="px-1.5 py-0.5 sm:px-3 sm:py-1 bg-gradient-to-r from-orange-500 to-red-500 text-white text-[10px] sm:text-xs font-bold rounded-full shadow-lg flex items-center gap-0.5 sm:gap-1 animate-pulse whitespace-nowrap">
                            🔥 <span className="hidden xs:inline">HOT</span>
                        </div>
                    )}

                    {isTrendingPost && !isHotPost && (
                        <div className="px-1.5 py-0.5 sm:px-3 sm:py-1 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-[10px] sm:text-xs font-bold rounded-full shadow-lg flex items-center gap-0.5 sm:gap-1 whitespace-nowrap">
                    ...
                    {poll && (
                        <div className="px-1.5 py-0.5 sm:px-3 sm:py-1 bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-[10px] sm:text-xs font-bold rounded-full shadow-lg flex items-center gap-0.5 sm:gap-1 whitespace-nowrap">
                    ...
                    {event && (
                        <div className="px-1.5 py-0.5 sm:px-3 sm:py-1 bg-gradient-to-r from-orange-500 to-yellow-500 text-white text-[10px] sm:text-xs font-bold rounded-full shadow-lg flex items-center gap-0.5 sm:gap-1 whitespace-nowrap">
                ...
                <div className="p-2.5 sm:p-4 flex items-start gap-2 sm:gap-3 relative">
                    <AnonAvatar authorId={post.author_id} size="sm" />
                    <div className="flex-1 min-w-0 pr-16 sm:pr-24">
                        <div className="flex items-center justify-between mb-0.5 sm:mb-1">
                            <div className="flex items-center gap-1 sm:gap-2 min-w-0 flex-1">
                                <div className={`font-semibold text-xs sm:text-base truncate ${post.author_name
                                ...
                                {post.mood && (
                                    <div className="hidden xs:block">
                                        <MoodBadge mood={JSON.parse(post.mood)} />
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {displayImages.slice(0, 4).map((url, idx) => (
                            ...
                            >
                                <img
                                    ...
                                    src={url}
                                    alt={`media ${idx + 1}`}
                                    className={`w-full object-cover transition-transform group-hover/img:scale-105 ${displayImages.length === 1 ? 'max-h-72 sm:max-h-96' : 'h-32 sm:h-48'
                                        }`}
                                />
                ...
                {post.media_type === 'video' && post.media_url && (
                    <div className="w-full">
                        <video
                            src={post.media_url}
                            className="w-full max-h-56 sm:max-h-96"
                            controls
                            preload="metadata"
                ...
                {post.media_type === 'audio' && post.media_url && (
                    <div className="px-2.5 sm:px-4 pb-2 sm:pb-3">
                        <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-4 bg-gradient-to-r from-purple-100 to-pink-100 dark:from-purple-900/30 dark:to-pink-900/30 rounded-lg sm:rounded-xl">
                            <div className="w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center flex-shrink-0">
                ...
                {event && (
                    <div className="px-2.5 sm:px-4 pb-2 sm:pb-3" onClick={(e) => e.stopPropagation()}>
                        <EventDisplay
                ...
                <div className="px-2.5 sm:px-4 py-2 sm:py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                    ...
                    <div className="flex items-center justify-between text-xs sm:text-sm">
                        <div className="flex items-center gap-1 sm:gap-3">
                            <button
                                ...
                                className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 text-gray-600 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all touch-manipulation active:scale-95"
                            >
                                <Heart className="w-3.5 h-3.5 sm:w-5 sm:h-5" />
                                <span className="font-medium hidden xs:inline">React</span>
                            </button>
                            <button
                                onClick={(e) => {
                                    e.stopPropagation()
                                ...
                                className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 text-gray-600 dark:text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-all touch-manipulation active:scale-95"
                            >
                        ...
                        <div className="flex items-center gap-0.5 sm:gap-1">
                            <button
                                ...
                                className={`flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg transition-all touch-manipulation active:scale-95 ${linkCopied
                            ...
                            >
                                {linkCopied ? (
                                    ...
                                    <LinkIcon className="w-3.5 h-3.5 sm:w-5 sm:h-5" />
                                )}
                            </button>
                            <button
                                onClick={handleReport}
                                disabled={isReported}
                                className={`flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg transition-all touch-manipulation active:scale-95 ${isReported
