...
export default function AdminPanel() {
    ...
    const handleAdStatus = async (adId, newStatus) => {
        try {
            ...
            if (error) throw error;
            setAds(ads.map(ad => ad.id === adId ? { ...ad, status: newStatus, rejection_reason: null } : ad));
        } catch (error) {
    ...
    const handleRejectAd = async (adId) => {
        const reason = window.prompt("Reason for rejection (this will be shown to the user):", "Invalid content or payment not received.");
        if (!reason) return;
        ...
        try {
            ...
            if (error) throw error;
            setAds(ads.map(ad => ad.id === adId ? { ...ad, status: 'rejected', rejection_reason: reason } : ad));
        } catch (error) {
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transform transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="h-full flex flex-col">
                    ...
                    <div className="p-4 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <button onClick={signOut} className="w-full flex items-center gap-3 px-4 py-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:bg-red-900/20 rounded-xl transition">
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                                <div className="block md:hidden divide-y divide-gray-100 dark:divide-gray-700">
                                    {ads.length === 0 ? (
                                        <div className="p-6 text-center text-gray-500 dark:text-gray-400 text-sm">No advertisements found.</div>
                                    ) : ads.map(ad => (
                                        <div key={ad.id} className="p-4 space-y-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                            <div className="flex justify-between items-start gap-3">
                                                <div className="flex gap-3 min-w-0">
                                                    <a href={ad.poster_url} target="_blank" rel="noreferrer" className="shrink-0">
                                                        <img src={ad.poster_url} alt="Ad" className="w-16 h-16 object-cover rounded-xl border border-gray-200 dark:border-gray-700" />
                                                    </a>
                                                    <div className="min-w-0 flex flex-col justify-center">
                                                        <h4 className="font-bold text-gray-900 dark:text-white truncate">{ad.brand_name || 'Anonymous Brand'}</h4>
                                                        <div className="text-xs text-gray-500 mt-0.5">{new Date(ad.created_at).toLocaleDateString()}</div>
                                                        <span className={`mt-1.5 inline-flex items-center w-max px-2 py-0.5 rounded-lg text-[10px] font-bold border ${ad.status === 'approved' ? 'bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800/50' :
                                                ...
                                                <div className="text-right shrink-0">
                                                    <div className="text-gray-900 dark:text-white font-extrabold text-lg">RM{ad.amount_paid}</div>
                                                    <div className="text-[10px] text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded mt-0.5 inline-block font-bold">
                                                        {ad.package_type === 'feed_3days' ? '3 Days' : ad.package_type === 'feed_1week' ? '1 Week' : '2 Weeks'}
                                                    </div>
                                                </div>
                                            </div>

                                            <div className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2 bg-gray-50 dark:bg-gray-900/50 p-2 rounded-lg italic">
                                                "{ad.caption}"
                                            </div>

                                            {ad.rejection_reason && (
                                                <div className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-2.5 rounded-lg border border-red-100 dark:border-red-900/30">
                                                    <strong>Rejected Reason:</strong> {ad.rejection_reason}
                                                </div>
                                            )}

                                            <div className="flex gap-2 pt-1 border-t border-gray-100 dark:border-gray-700">
                                                {ad.status !== 'approved' && (
                                                    <button onClick={() => handleAdStatus(ad.id, 'approved')} className="flex-1 py-2 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded-lg text-xs font-bold flex justify-center items-center gap-1.5 hover:bg-green-100 transition-colors">
                                                        <CheckCircle className="w-3.5 h-3.5" /> Approve
                                                    </button>
                                                )}
                                                {ad.status !== 'rejected' && (
                                                    <button onClick={() => handleRejectAd(ad.id)} className="flex-1 py-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg text-xs font-bold flex justify-center items-center gap-1.5 hover:bg-red-100 transition-colors">
                                                        <XCircle className="w-3.5 h-3.5" /> Reject
                                                    </button>
                                                )}
                                                <button onClick={() => deleteAd(ad.id)} className="p-2 bg-gray-50 dark:bg-gray-900 text-gray-400 hover:text-red-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                                                    <Trash2 className="w-4 h-4" />
                                                </button>
                                            </div>
                                        </div>
                                                ...
                                                <tr key={ad.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                                    <td className="px-6 py-4">
                                                        <a href={ad.poster_url} target="_blank" rel="noreferrer">
                                                            <img src={ad.poster_url} alt="Ad" className="w-20 h-20 object-cover rounded-xl border border-gray-200 dark:border-gray-700 hover:opacity-80 transition-opacity" />
                                                        </a>
                                                    </td>
                                                    <td className="px-6 py-4 max-w-xs">
                                                        <p className="text-gray-900 dark:text-white font-bold mb-1">{ad.brand_name || 'Anonymous Brand'}</p>
                                                        <p className="text-gray-700 dark:text-gray-300 text-xs line-clamp-2 mb-2 italic">"{ad.caption}"</p>
                                                        <div className="flex flex-wrap gap-2">
                                                            {ad.whatsapp_link && (
                                                                <a href={ad.whatsapp_link} target="_blank" rel="noreferrer" className="text-green-600 dark:text-green-400 font-medium text-xs hover:underline flex items-center">
                                                                    <MessageSquare className="w-3 h-3 mr-1" /> WhatsApp
                                                                </a>
                                                            )}
                                                            {ad.website_link && (
                                                                <a href={ad.website_link} target="_blank" rel="noreferrer" className="text-blue-600 dark:text-blue-400 font-medium text-xs hover:underline flex items-center">
                                                                    <LinkIcon className="w-3 h-3 mr-1" /> Link
                                                                </a>
                                                            )}
                                                        </div>
                                                        <p className="text-gray-400 text-[10px] mt-2 flex items-center">
                                                            <Clock className="w-3 h-3 mr-1" /> {new Date(ad.created_at).toLocaleDateString()}
                                                        </p>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-[10px] font-bold bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 mb-2 border border-blue-200 dark:border-blue-800/50">
                                                            {ad.package_type === 'feed_3days' ? 'Pinned (3 Days)' : ad.package_type === 'feed_1week' ? 'Pinned (1 Week)' : 'Pinned (2 Weeks)'}
                                                        </span>
                                                        <div className="text-gray-900 dark:text-white font-extrabold text-xl">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex flex-col items-start gap-2">
                                                            ...
                                                            {ad.rejection_reason && (
                                                                <span className="text-[10px] text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded border border-red-100 dark:border-red-900/30 max-w-[150px]">
                                                                    {ad.rejection_reason}
                                                                </span>
                                                            )}
                                                        </div>
