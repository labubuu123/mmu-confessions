...
import { X, MessageCircle, Calendar, Tag, AlertCircle, ChevronLeft, ChevronRight, Trash2, Loader2 } from 'lucide-react';
...
export default function MarketplaceItemModal({ item, onClose, onDelete }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="bg-white dark:bg-gray-800 w-full max-w-2xl max-h-[90vh] rounded-3xl shadow-2xl overflow-hidden relative flex flex-col animate-in fade-in zoom-in-95 duration-200">
                <div className="absolute top-4 right-4 z-10 flex gap-2">
                    {isOwner && (
                        <button
                            onClick={handleDelete}
                            disabled={isDeleting}
                            className="p-2 bg-white/90 dark:bg-black/60 text-red-500 rounded-full hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors backdrop-blur-md shadow-sm"
                        >
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 bg-white/90 dark:bg-black/60 text-gray-500 dark:text-gray-300 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors backdrop-blur-md shadow-sm"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="overflow-y-auto flex-1 overscroll-contain">
                    <div className="relative aspect-video sm:aspect-[2/1] bg-gray-100 dark:bg-gray-900">
                        ...
                        {hasMultipleImages && (
                            <>
                                <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 p-1.5 bg-black/30 hover:bg-black/50 text-white rounded-full backdrop-blur-md transition-all">
                                    <ChevronLeft className="w-6 h-6" />
                                </button>
                                <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 p-1.5 bg-black/30 hover:bg-black/50 text-white rounded-full backdrop-blur-md transition-all">
